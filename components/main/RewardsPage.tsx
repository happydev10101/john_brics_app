          ...
          <>
            ...
            {tierLevel && tierLevel.level > 0 && tierLevel.additionalBenifits && tierLevel.additionalBenifits.length > 0 && (
              <MyCard p={0} className="overflow-hidden">
                ...
                <div className="divide-y divide-info">
                  {tierLevel.dailyRewardsPercent > 0 && (
                    <div className="flex items-start space-x-3 p-4">
                      <span className="text-xl">🔴</span>
                      <p className="text-sm text-gray-300 flex-1">
                        {tierLevel.dailyRewardsPercent.toLocaleString()}% Daily Rewards
                      </p>
                    </div>
                  )}
                  {tierLevel.additionalBenifits.map((benefit, index) => (
                    <div key={index} className="flex items-start space-x-3 p-4">
                      ...
                      <div className="text-sm text-gray-300 flex-1 space-y-2">
                        ...
                        {benefit.subItems && benefit.subItems.length > 0 && (
                          <div className="space-y-1 pl-2 border-l border-info/40">
                            {benefit.subItems.map((sub, subIndex) => (
                              <div key={subIndex} className="flex items-start space-x-2">
                                <span className="text-base">{sub.emoticon}</span>
                                <p className="text-sm text-gray-300" dangerouslySetInnerHTML={{ __html: sub.text }} />
                              </div>
                            ))}
                          </div>
                        )}
