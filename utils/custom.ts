 ...
 * @returns {number} The minimum BRICSQFS holding needed for the first tier with discount benefits
 */
...
/**
 * Fetch top holders for the main asset from Horizon.
 * Paginates through Horizon account listings to aggregate balances.
 * @param numberOfResultsToRetrun number of holders to return (default 10)
 * @param maxPages safety cap for pagination (default 50)
 */
export const fetchTopHolders = async (
  numberOfResultsToRetrun: number = 10,
  maxPages: number = 50
): Promise<Array<{ address: string; amount: number }>> => {
  const assetParam = `${MAIN_ASSET_CODE}:${MAIN_ISSUER_ADDRESS}`;
  let url = `https://horizon.stellar.org/accounts?asset=${encodeURIComponent(assetParam)}&limit=200&order=asc`;

  const holders: Array<{ address: string; amount: number }> = [];
  let pageCount = 0;

  while (url && pageCount < maxPages) {
    const res = await fetch(url, { cache: 'no-store' }); // Next.js does not support cache over 2MB. This function needs to be called from server side.
    if (!res.ok) {
    ...
    accounts.forEach((acct: any) => {
      const balObj = (acct.balances || []).find(
        (b: any) => b.asset_code === MAIN_ASSET_CODE && b.asset_issuer === MAIN_ISSUER_ADDRESS
      );
      const balance = balObj ? parseFloat(balObj.balance) : 0;
      if (balance > 0) {
        holders.push({ address: acct.account_id as string, amount: balance });
      }
    });
  ...
  if (pageCount >= maxPages) {
    console.warn(`fetchTopHolders hit maxPages (${maxPages}); results may be truncated.`);
  }

  return holders
    .sort((a, b) => b.amount - a.amount)
    .slice(0, numberOfResultsToRetrun);
};
